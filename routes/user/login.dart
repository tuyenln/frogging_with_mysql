...
Future<Response> onRequest(RequestContext context) async {
  if (context.request.method.value != 'POST') {
    return Response.json(body: {'code': 400, 'status': 'fail', 'message': 'Method do not allow'});
  }
  ...
  final users = await dataRepository.fetchUser(email, password);

  if (users.isEmpty) {
    return Response.json(body:{'status': 'fail','code': 200, 'message': 'fail'});
  }

  // than we will return the response as JSON
  return Response.json(body: {'status': 'ok','code': 200, 'message': 'ok', 'data':  users});
}
